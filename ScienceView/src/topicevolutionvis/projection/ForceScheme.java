...
public class ForceScheme {
    ...
    public ForceScheme(float fractionDelta, int numberPoints) {
        ...
        for (int ind = 0, j = 0; j < this.index.length; ind += index_aux.size() / 10, j++) {
            ...
            index_aux.removeAt(ind);
        }
    }

    public double iteration(DistanceMatrix dmat, double[][] projection) {
        double error = 0.0f;
...
//                delta = (double) Math.sqrt(Math.abs(delta));
    ...
    public double iteration(SparseMatrix matrix, Dissimilarity diss, double[][] projection) {
        double error = 0.0f;
        ...
        if (projection[0].length == 2) {
            ...
            for (int ins1 = 0; ins1 < projection.length; ins1++) {
                ...
                for (int ins2 = 0; ins2 < projection.length; ins2++) {
                    ...
                    double drn = diss.calculate(matrix.getRowWithIndex(instance), matrix.getRowWithIndex(instance2));
...
//            SparseMatrix matrix = PExMatrixFactory.getInstance(filename);
//
//            float[][] projection = new float[matrix.getRowsCount()][];
//
...
//            }
//
//            ForceScheme force = new ForceScheme(8.0f, matrix.getRowsCount());
//
//            for (int i = 0; i < 1; i++) {
