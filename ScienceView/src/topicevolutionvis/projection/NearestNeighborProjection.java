...
public class NearestNeighborProjection extends Projector {
    ...
    public double[][] project(DistanceMatrix dmat) {
        ...
        for (int x = 2; x < dmat.getElementCount(); x++) {
            ...
            if (number == 1) {
                ...
                double distanceQX1 =  Math.sqrt(((projection[q][0] - resultingIntersections.first.x) * (projection[q][0] - resultingIntersections.first.x)) +
                        ((projection[q][1] - resultingIntersections.first.y) * (projection[q][1] - resultingIntersections.first.y)));

                double distanceQX2 =  Math.sqrt(((projection[q][0] - resultingIntersections.second.x) * (projection[q][0] - resultingIntersections.second.x)) +
                        ((projection[q][1] - resultingIntersections.second.y) * (projection[q][1] - resultingIntersections.second.y)));

                double distanceRX1 =  Math.sqrt(((projection[r][0] - resultingIntersections.first.x) * (projection[r][0] - resultingIntersections.first.x)) +
                        ((projection[r][1] - resultingIntersections.first.y) * (projection[r][1] - resultingIntersections.first.y)));

                double distanceRX2 =  Math.sqrt(((projection[r][0] - resultingIntersections.second.x) * (projection[r][0] - resultingIntersections.second.x)) +
    ...
    private int intersect(Circle circle1, Circle circle2, Pair intersect) {
        double lvdistance = Math.sqrt(((circle1.center.x - circle2.center.x) * (circle1.center.x - circle2.center.x)) +
