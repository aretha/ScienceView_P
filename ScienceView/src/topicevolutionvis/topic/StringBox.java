...
public class StringBox {
    ...
    public java.awt.Rectangle draw(Graphics g, java.awt.Point position, java.awt.Font font, boolean selected, boolean useGrayColor) {
        java.awt.Graphics2D g2 = (java.awt.Graphics2D) g;

        String print_msg = msg;
        g2.setFont(font.deriveFont(Font.BOLD));
        if (msg.length() > 40) {
            int weight_delimiter = msg.indexOf("[");
            String weight = msg.substring(weight_delimiter);
            int cut_point = msg.indexOf(",", 40);
            if (cut_point == -1 || cut_point > weight_delimiter) {
                print_msg = msg;
            } else {
                print_msg = new StringBuilder(msg.substring(0, cut_point)).append(",...)").append(weight).toString();
            }
        ...
        int width = metrics.stringWidth(print_msg);
        ...
        g2.drawString(print_msg, position.x, position.y + height - 2);
