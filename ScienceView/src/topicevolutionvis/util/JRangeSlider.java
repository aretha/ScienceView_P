...
import topicevolutionvis.util.rangeslider.NumberRangeModel;
...
public class JRangeSlider extends JComponent
    ...
    final public static int PREFERRED_LENGTH = 170;
    ...
    protected Color thumbColor = Color.gray;
    ...
    public JRangeSlider(float minimum, float maximum, float lowValue, float highValue, int orientation) {
    ...
    public JRangeSlider(BoundedRangeModel model, int orientation, int direction) {
        ...
        Dimension sliderSize = new Dimension(PREFERRED_LENGTH, PREFERRED_BREADTH);

        this.setSize(sliderSize);
        this.setMinimumSize(sliderSize);
        this.setMaximumSize(sliderSize);

        setForeground(Color.LIGHT_GRAY);

        this.lstnr = new RangeSliderChangeListener();
    ...
    public void setEnabled(boolean enabled) {
        if (enabled) {
            ...
            this.setThumbColor(Color.lightGray);
        }
    }
    ...
    public void keyPressed(KeyEvent e) {
        ...
        } else if ((v && u || !v && r) && highValue + increment <= maximum) {
            offset(increment);
        }
    }
